...
const Jackets = ({ products }) => {
  ...
  return (
    <div><section className="text-gray-600 body-font">
       <Head>
          <title>Jackets-CodeWear</title>
          </Head>
      <div className="container px-5 mt-3 md:m-8 mx-auto">
        <div className="flex flex-wrap m-4 ">
          
          {Object.keys(products).length === 0 && <p>Soory! all the Jackets are currently out of stock</p>}
          {
...
export async function getServerSideProps(context) {
  let products = await fetch('http://localhost:3000/api/product?category=jacket');

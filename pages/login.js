...
const Login = () => {
  ...
  return (
    <div className="min-h-full flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      ...
      />
       <Head>
          <title>Login-CodeWear</title>
          </Head>
      <div className="max-w-md w-full space-y-8">
        <div>
          <div className="flex justify-center">
            <Image className="mx-auto h-12 w-auto rounded" src="/wear.png" alt="Workflow" width={50} height={50}></Image>
          </div>
          <h2 className="mt-6 text-center text-3xl tracking-tight font-bold text-gray-900">Sign in to your account</h2>
          <p className="mt-2 mx-2 text-center text-sm text-gray-600">
            Or
            <Link className='' href='/signup'><a href="#" className="mx-4 font-medium text-pink-600 hover:text-pink-500">Signup</a></Link>
          </p>
        </div>
        <form onSubmit={handleSubmit} className="mt-8 space-y-6" method="POST">
          ...
          <div className="rounded-md shadow-sm -space-y-px">
            ...
            <div>
              ...
              <input onChange={handleChange} value={password} id="password" name="password" type="password" required className="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-b-md focus:outline-none focus:ring-pink-500 focus:border-pink-500 focus:z-10 sm:text-sm" placeholder="Password" />
            </div>
          </div>
